...
const Compliance: React.FC = () => {
    ...
    // Generate large mock data for virtual scrolling demo
    const docData = React.useMemo(() => Array.from({ length: 1000 }, (_, i) => ({
        id: `V-${1000 + i}`,
        plate: `XX-YY-${String(i).padStart(3, '0')}`,
        revTecnica: '2024-12-15',
        perCirculacion: '2025-03-31',
        status: i % 3 === 0 ? 'Valid' : i % 3 === 1 ? 'Expiring Soon' : 'Expired'
    })), []);
    ...
    const renderDocRow = (doc: any) => {
        return (
            <div key={doc.id} className="flex items-center hover:bg-white/5 transition-colors border-b border-white/5">
                ...
                <div className="p-4 w-1/4 font-mono text-sm text-slate-300">{doc.revTecnica}</div>
                <div className="p-4 w-1/4 font-mono text-sm text-slate-300">{doc.perCirculacion}</div>
                                ...
                                {laborData.map(driver => (
                                    <div key={driver.id}>
                                        ...
                                        <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full rounded-full ${driver.status === 'Critical' ? 'bg-red-500' :
                        ...
                        <div className="glass-panel border border-white/5 rounded-2xl p-8 relative overflow-hidden animate-slide-up" style={{ animationDelay: '0.3s' }}>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
                                {isoSteps.map((step, idx) => (
                                    <div key={idx} className={`p-4 rounded-xl border transition-all ${step.completed
