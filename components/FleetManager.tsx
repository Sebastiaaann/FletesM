...
const FleetManager: React.FC = () => {
  ...
  const handleSave = async () => {
    const newErrors: any = {};
    if (activeTab === 'vehicles') {
      if (!formData.plate) newErrors.plate = ERROR_MESSAGES.PLATE_REQUIRED;
  ...
  const runPrediction = async (vehicle: Vehicle) => {
    ...
    try {
      ...
      showToast.error("Error al obtener predicción");
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-dark-900 border border-white/10 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-slide-up">
            ...
            <div className="p-6 space-y-4">
              {activeTab === 'vehicles' ? (
                <>
                  <Input label="Patente" value={formData.plate} onChange={(e: any) => setFormData({ ...formData, plate: e.target.value })} error={errors.plate} placeholder="XX-YY-12" />
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Input label="Fecha Vencimiento" type="date" value={formData.licenseExpiry} onChange={(e: any) => setFormData({ ...formData, licenseExpiry: e.target.value })} error={errors.licenseExpiry} />
                  </div>
                </>
              )}
            </div>
